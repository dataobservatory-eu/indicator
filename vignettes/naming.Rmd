...
`frequency`: Frequency of observation, currently *A*nnual, *Q*uarterly, *M*onthly, *D*aily, or unknown.
`estimate`: See below.
...
`missing`: Missing data, which is not filled by *locf*, *nocb*, *interpolate*, *forecast*, *backcaskt* or *impute*.
...
`impute`: An imputation technique was used that is not based on time series, but cross-selctional, geographical or other information. This is likely to be rather specific to the actual indicator.
...
`impute`: number of values filled with any other *imputation* method that is not based on time series, but cross-selctional, geographical or other information.
